...
| `transport` | `string` | `"stdio"`, `"tcp"`, `"unix"`, `"mem"`, `"ws"`, `"wss"` | Wire transport |
...
| `mem://` | **Mono** | Full | **1** | **No** | The in-process channel pair |
| `ws://` | Multi | Full | N | Yes | Per WebSocket connection |
| `wss://` | Multi | Full | N | Yes | Per WebSocket connection (TLS) |

### Valence Constraints

**Monovalent transports** (`stdio://`, `mem://`) have exactly one session
per holon lifetime. The implications:

- **No session list** — there is at most one active session. The ring
  buffer may contain zero or one past session (from a reconnect attempt
  that failed before the current one succeeded — unlikely but possible
  for `mem://` in test setups).
...
For local transports (`stdio://`, `mem://`), these fields are empty.
...
|---|---|---|
| `full` | All fields returned | `stdio://`, `mem://` (local-only transports) |
