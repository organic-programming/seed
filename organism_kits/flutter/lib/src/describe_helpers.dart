...
void ensureCoreDescribeServices(
  DescribeResponse response, {
  String? coaxDescription,
}) {
  if (!response.services.any(
    (service) => service.name == 'holons.v1.CoaxService',
  )) {
    response.services.add(
      coaxServiceDocumentation(description: coaxDescription),
    );
  }
  if (!response.services.any(
    (service) => service.name == 'holons.v1.HolonMeta',
  )) {
    response.services.add(holonMetaServiceDocumentation());
  }
}

ServiceDoc holonMetaServiceDocumentation() {
  return ServiceDoc(
...
ServiceDoc coaxServiceDocumentation({String? description}) {
  return ServiceDoc(
    name: 'holons.v1.CoaxService',
    description:
        description ??
        'COAX interaction surface for organism apps. It exposes member discovery, connection, and app-level orchestration through shared runtime state.',
    methods: <MethodDoc>[
      MethodDoc(
        name: 'ListMembers',
        description:
        ...
        outputFields: <FieldDoc>[
          FieldDoc(
            ...
            nestedFields: memberInfoFieldDocumentation(),
          ),
        ],
        exampleInput: '{}',
      ),
...
ServiceDoc appServiceDocumentation({
  required String name,
  String description = '',
  Iterable<MethodDoc> methods = const <MethodDoc>[],
}) {
  return ServiceDoc(
    name: name,
...
List<FieldDoc> memberInfoFieldDocumentation() {
  return <FieldDoc>[
    FieldDoc(
      name: 'slug',
      type: 'string',
    ...
    FieldDoc(
      ...
      label: FieldLabel.FIELD_LABEL_OPTIONAL,
      nestedFields: identityFieldDocumentation(),
    ),
    FieldDoc(
      name: 'state',
      ...
      label: FieldLabel.FIELD_LABEL_OPTIONAL,
      enumValues: memberStateValueDocumentation(),
    ),
    FieldDoc(
      name: 'is_organism',
      type: 'bool',
...
List<FieldDoc> identityFieldDocumentation() {
  return <FieldDoc>[
    FieldDoc(
      name: 'given_name',
      type: 'string',
...
List<EnumValueDoc> memberStateValueDocumentation() {
  return <EnumValueDoc>[
    EnumValueDoc(
