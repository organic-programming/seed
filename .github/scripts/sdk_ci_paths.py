...
ZIG_GRPC_DOWNSTREAM = ["zig", *CPP_DOWNSTREAM]
...
REPUBLISH_ALL_PATHS = {
    ...
    ".github/scripts/seed_release_bump.py",
...
def is_sdk_doc_or_asset(path: str) -> bool:
    ...
    if suffix in SDK_SOURCE_EXCLUDE_EXTENSIONS:
        return True
    if lower_name.startswith(("readme", "license", "changelog")):
        return True
    return "docs" in {part.lower() for part in parts}
...
def is_sdk_source_path(path: str) -> bool:
    ...
    if path.startswith("holons/grace-op/cmd/protoc-gen-op-adapter/"):
        return not path.lower().endswith(".md")
    if path.startswith("holons/grace-op/cmd/protoc-gen-op-noop/"):
        return not path.lower().endswith(".md")
    return False
...
def publish_set(paths: list[str]) -> list[str]:
    ...
    for path in normalized:
        ...
        if path.startswith("sdk/cpp-holons/"):
            ...
            groups.append(ZIG_GRPC_DOWNSTREAM)
        else:
