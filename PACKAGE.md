...
| `PROTO.md` | Manifest authoring spec. Canonical format. This spec defines how the package is produced from it. |
...
> Detailed semantics of `op list`, and `op inspect` are defined in `OP.md`.
...
For the strict resolution priority (Siblings → CWD → Source, etc.), see the cross-SDK [DISCOVERY.md](../../DISCOVERY.md) spec.

For packaged layers (built, installed, cached), `op` reads `.holon.json` for fast identity
