...
RUBY_NODE = File.expand_path("../cascade-node-ruby", HERE)
...
def run_default
  ...
  run_root = Pathname.new(Dir.mktmpdir("relay-cascade-ruby-"))
  puts "=== relay-cascade-ruby ==="
  puts
  total_pass = 0
  total_fail = 0
  previous = ""
...
def run_live_stream
  binary = find_binary(RUBY_SLUG)
  run_root = Pathname.new(Dir.mktmpdir("relay-cascade-ruby-live-"))
  puts "=== relay-cascade-ruby --live-stream ==="
  puts
...
def run_multi_pattern
  ...
  run_root = Pathname.new(Dir.mktmpdir("relay-cascade-ruby-multi-"))
  puts "=== relay-cascade-ruby (multi-pattern) ==="
  puts
  total_pass = 0
  total_fail = 0
...
def new_role_runtime(phase, transport, role, spec)
  ...
  when "unix"
    path = "/tmp/relay-cascade-ruby-p#{phase}-#{role.downcase}-#{Process.pid}.sock"
  ...
  RoleRuntime.new(
    ...
    member_slug: "",
    metrics_addr: "",
    stderr_path: File.join(Dir.tmpdir, "relay-cascade-ruby-#{phase}-#{role.downcase}-#{Process.pid}.stderr")
  )
end
...
def find_binary(slug)
  env_name = "CASCADE_NODE_#{slug.delete_prefix('cascade-node-').upcase.tr('-', '_')}_BIN"
