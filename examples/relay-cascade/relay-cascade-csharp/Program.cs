...
sealed class App
{
    ...
    private const string csharpSlug = "cascade-node-csharp";
    private const string goSlug = "cascade-node-go";
    ...
    public async Task RunDefault()
    {
        ...
        var runRoot = Directory.CreateTempSubdirectory("relay-cascade-csharp-").FullName;
        Console.WriteLine("=== relay-cascade-csharp ===");
        Console.WriteLine();
        var totalPass = 0;
        var totalFail = 0;
        var previous = "";
    ...
    {
        var binary = await FindBinary(csharpSlug);
        var runRoot = Directory.CreateTempSubdirectory("relay-cascade-csharp-live-").FullName;
        Console.WriteLine("=== relay-cascade-csharp --live-stream ===");
        Console.WriteLine();
    ...
    public async Task RunMultiPattern()
    {
        ...
        var runRoot = Directory.CreateTempSubdirectory("relay-cascade-csharp-multi-").FullName;
        Console.WriteLine("=== relay-cascade-csharp (multi-pattern) ===");
        Console.WriteLine();
        var totalPass = 0;
        var totalFail = 0;
    ...
    private RoleRuntime NewRoleRuntime(int phase, string transport, string role, RoleSpec spec)
    {
        ...
        if (transport == "unix")
        {
            var socket = $"/tmp/relay-cascade-csharp-p{phase}-{role.ToLowerInvariant()}-{Environment.ProcessId}.sock";
    ...
    {
        var envName = $"CASCADE_NODE_{slug["cascade-node-".Length..].ToUpperInvariant().Replace('-', '_')}_BIN";
    ...
    private static string FindRelayRoot()
    {
        ...
        {
            if (Directory.Exists(Path.Combine(current.FullName, "cascade-node-csharp")) &&
                Directory.Exists(Path.Combine(current.FullName, "cascade-node-go")))
                return current.FullName;
            current = current.Parent;
        }
        throw new InvalidOperationException("relay-cascade root not found");
    }
