...
const NODE_NODE = path.resolve(HERE, '../cascade-node-node');
const SDK_ROOT = path.join(ROOT, 'sdk', 'js-holons');

process.env.NODE_PATH = [
  path.join(NODE_NODE, '.op', 'build', 'npm', 'node_modules'),
...
const NODE_SLUG = 'cascade-node-node';
const GO_SLUG = 'cascade-node-go';
...
class RoleRuntime {
  constructor({ role, uid, slug, binaryPath, listenUri, relayAddress, clientTarget }) {
    ...
    this.obsClient = null;
    this.stderrPath = path.join(os.tmpdir(), `relay-cascade-node-${process.pid}-${uid}.stderr`);
  }
}
...
async function runDefault() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'relay-cascade-node-'));
  console.log('=== relay-cascade-node ===');
  console.log();
  let totalPass = 0;
  let totalFail = 0;
  let previous = '';
...
async function runLiveStream() {
  const binary = findBinary(NODE_SLUG);
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'relay-cascade-node-live-'));
  console.log('=== relay-cascade-node --live-stream ===');
  console.log();
...
async function runMultiPattern() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), 'relay-cascade-node-multi-'));
  console.log('=== relay-cascade-node (multi-pattern) ===');
  console.log();
  let totalPass = 0;
  let totalFail = 0;
...
function newRoleRuntime(phase, transport, role, spec) {
  ...
  if (transport === 'unix') {
    const socketPath = `/tmp/relay-cascade-node-p${phase}-${role.toLowerCase()}-${process.pid}.sock`;
...
async function startRole(cascade, runtime) {
  ...
  try {
    ...
    throw new Error(`start ${runtime.role}: ${stderrText || error.message}`);
  }
}
...
function findBinary(slug) {
  const envName = `CASCADE_NODE_${slug.replace(/^cascade-node-/, '').toUpperCase().replace(/-/g, '_')}_BIN`;
