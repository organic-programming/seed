...
constexpr const char *kCSlug = "cascade-node-c";
...
std::string relay_root_from(const std::filesystem::path &start) {
  ...
  while (!current.empty()) {
    if (std::filesystem::exists(current / "cascade-node-go") &&
        std::filesystem::exists(current / "cascade-node-c")) {
      return current.string();
    }
    auto parent = current.parent_path();
    if (parent == current) break;
...
RoleRuntime new_role_runtime(int phase, const std::string &transport,
  ...
  if (transport == "tcp") {
    ...
    auto path = "/tmp/relay-cascade-c-p" + std::to_string(phase) + "-" +
...
void run_default(const std::string &c_binary, const std::string &repo_root) {
  auto run_root = make_temp_dir("relay-cascade-c-");
  std::cout << "=== relay-cascade-c ===\n\n";
  int pass = 0, fail = 0;
  std::string previous;
...
void run_live_stream(const std::string &c_binary, const std::string &repo_root) {
  auto run_root = make_temp_dir("relay-cascade-c-live-");
  std::cout << "=== relay-cascade-c --live-stream ===\n\n";
...
void run_multi_pattern(const std::string &c_binary, const std::string &go_binary,
  ...
  auto run_root = make_temp_dir("relay-cascade-c-multi-");
  std::cout << "=== relay-cascade-c (multi-pattern) ===\n\n";
  int pass = 0, fail = 0;
