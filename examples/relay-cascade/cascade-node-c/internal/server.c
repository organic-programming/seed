...
static relay_v1_TickResponse *cascade_node_c_handle_tick(
  ...
  relay_v1_TickResponse_set_responder_slug(
      response, arena_copy_cstr(arena, "cascade-node-c"));
...
int cascade_node_c_serve(const char *listen_uri,
  ...
  obs_options.slug = "cascade-node-c";
  ...
  obs_config.slug = "cascade-node-c";
  ...
  rc = cascade_node_c_generated_serve(listen_uri, &handlers, &serve_options,
                                      err, sizeof(err));
