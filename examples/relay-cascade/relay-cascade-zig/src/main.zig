...
const zig_slug = "cascade-node-zig";
...
pub fn main(init: std.process.Init.Minimal) !void {
    ...
    if (std.mem.eql(u8, mode, "--multi-pattern")) {
        ...
        try runMode(allocator, "=== relay-cascade-zig --live-stream ===", "Summary: {} PASS / {} FAIL across {} ticks", true);
    } else {
        try runMode(allocator, "=== relay-cascade-zig ===", "Summary: {} ticks, {} PASS, {} FAIL", false);
    }
}
...
fn runMultiPattern(allocator: std.mem.Allocator) !void {
    ...
    try print("=== relay-cascade-zig (multi-pattern) ===\n\n", .{});
    var pass: usize = 0;
    var fail: usize = 0;
...
fn listenUri(allocator: std.mem.Allocator, phase: usize, transport: []const u8, role: []const u8) ![]u8 {
    ...
    const path = try std.fmt.allocPrint(allocator, "/tmp/relay-cascade-zig-p{}-{s}.sock", .{ phase, lowerRole(role) });
    defer allocator.free(path);
