...
constexpr const char *kCppSlug = "cascade-node-cpp";
constexpr const char *kGoSlug = "cascade-node-go";
...
std::string relay_root_from(const std::filesystem::path &start) {
  auto current = std::filesystem::absolute(start);
  while (!current.empty()) {
    if (std::filesystem::exists(current / "cascade-node-go") &&
        std::filesystem::exists(current / "cascade-node-cpp")) {
      return current.string();
    }
    auto parent = current.parent_path();
    if (parent == current) break;
  ...
  throw std::runtime_error("relay-cascade root not found");
}
...
std::string find_binary(const std::string &slug, const std::string &relay_root) {
  auto suffix = slug.substr(std::string("cascade-node-").size());
  for (auto &ch : suffix) {
    ch = ch == '-' ? '_' : static_cast<char>(std::toupper(static_cast<unsigned char>(ch)));
  }
  if (const char *env = std::getenv(("CASCADE_NODE_" + suffix + "_BIN").c_str());
      env != nullptr && *env != '\0') {
    return trim(env);
  }
...
RoleRuntime new_role_runtime(int phase, const std::string &transport,
  ...
  if (transport == "tcp") {
    ...
    auto path = "/tmp/relay-cascade-cpp-p" + std::to_string(phase) + "-" +
...
void run_default(const std::string &cpp_binary, const std::string &repo_root) {
  auto run_root = make_temp_dir("relay-cascade-cpp-");
  std::cout << "=== relay-cascade-cpp ===\n\n";
  int pass = 0, fail = 0;
  std::string previous;
...
void run_live_stream(const std::string &cpp_binary, const std::string &repo_root) {
  auto run_root = make_temp_dir("relay-cascade-cpp-live-");
  std::cout << "=== relay-cascade-cpp --live-stream ===\n\n";
...
void run_multi_pattern(const std::string &cpp_binary, const std::string &go_binary,
  ...
  std::vector<Pattern> patterns = {
      {"cpp-cpp-cpp-cpp", all_specs(kCppSlug, cpp_binary)},
      {"cpp-cpp-go-cpp",
       {{"A", {kCppSlug, cpp_binary}}, {"B", {kCppSlug, cpp_binary}},
        {"C", {kGoSlug, go_binary}}, {"D", {kCppSlug, cpp_binary}}}},
      {"cpp-cpp-go-go",
       {{"A", {kCppSlug, cpp_binary}}, {"B", {kCppSlug, cpp_binary}},
        {"C", {kGoSlug, go_binary}}, {"D", {kGoSlug, go_binary}}}},
  };
  auto run_root = make_temp_dir("relay-cascade-cpp-multi-");
  std::cout << "=== relay-cascade-cpp (multi-pattern) ===\n\n";
  int pass = 0, fail = 0;
