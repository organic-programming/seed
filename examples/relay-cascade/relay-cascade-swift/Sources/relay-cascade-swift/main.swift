...
private let swiftSlug = "cascade-node-swift"
private let goSlug = "cascade-node-go"
...
private final class App {
    ...
    func runDefault() throws {
        ...
        let runRoot = try makeRunRoot(prefix: "relay-cascade-swift-")
        print("=== relay-cascade-swift ===")
        print()
        var totalPass = 0
        var totalFail = 0
        var previous = ""
    ...
    func runLiveStream() throws {
        let binary = try findBinary(swiftSlug)
        let runRoot = try makeRunRoot(prefix: "relay-cascade-swift-live-")
        print("=== relay-cascade-swift --live-stream ===")
        print()
    ...
    func runMultiPattern() throws {
        ...
        let runRoot = try makeRunRoot(prefix: "relay-cascade-swift-multi-")
        print("=== relay-cascade-swift (multi-pattern) ===")
        print()
    ...
    private func newRoleRuntime(phase: Int, transport: String, role: String, spec: RoleSpec) throws -> RoleRuntime {
        ...
        case "unix":
            let socket = "/tmp/relay-cascade-swift-p\(phase)-\(role.lowercased())-\(ProcessInfo.processInfo.processIdentifier).sock"
    ...
    private func findBinary(_ slug: String) throws -> String {
        let suffix = slug.replacingOccurrences(of: "cascade-node-", with: "").uppercased().replacingOccurrences(of: "-", with: "_")
        let envName = "CASCADE_NODE_\(suffix)_BIN"
    ...
    private static func findRelayRoot() throws -> String {
        ...
        while true {
            let directSwift = url.appendingPathComponent("cascade-node-swift", isDirectory: true).path
            let directGo = url.appendingPathComponent("cascade-node-go", isDirectory: true).path
            if FileManager.default.fileExists(atPath: directSwift), FileManager.default.fileExists(atPath: directGo) {
                return url.path
            }
            let nested = url.appendingPathComponent("examples/relay-cascade", isDirectory: true)
            if FileManager.default.fileExists(atPath: nested.appendingPathComponent("cascade-node-swift", isDirectory: true).path),
               FileManager.default.fileExists(atPath: nested.appendingPathComponent("cascade-node-go", isDirectory: true).path) {
                return nested.path
            }
        ...
        throw RuntimeError("relay-cascade root not found")
    }
