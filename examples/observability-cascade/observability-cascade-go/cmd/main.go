...
func runLiveStream() error {
	...
	fmt.Println("=== observability-cascade-go --live-stream ===")
	fmt.Println()
...
func newRoleRuntime(phase int, transportName, role string, spec roleSpec) *roleRuntime {
	...
	case "unix":
		path := fmt.Sprintf("/tmp/observability-cascade-%s.sock", lower)
		r.listenURIs = []string{"unix://" + path}
		r.clientTarget = "unix://" + path
		r.relayAddress = r.clientTarget
	case "stdio":
		sidecar := fmt.Sprintf("/tmp/observability-cascade-stdio-%s.sock", lower)
