...
// RunCLI dispatches the observability-cascade-node-go CLI and returns a process exit code.
func RunCLI(args []string, outputs ...io.Writer) int {
	...
	case "serve":
		...
		return 0
	case "version":
		fmt.Fprintf(stdout, "observability-cascade-node-go %s\n", VersionString())
		return 0
	case "help":
		printUsage(stdout)
		return 0
...
func printUsage(w io.Writer) {
	fmt.Fprintln(w, "usage: observability-cascade-node-go <command> [args] [flags]")
	fmt.Fprintln(w, "")
