...
void ensure_cascade_observability() {
  ...
  holons::observability::from_env(
      holons::observability::Config{"observability-cascade-c"});
}
...
std::vector<LanguageMember> own_language_members() {
  auto binary = member_binary("c-node");
  return {{"c", kCSlug, binary},
          {"c", kCSlug, binary},
          {"c", kCSlug, binary}};
}

std::vector<Pattern> multi_patterns() {
  auto c_binary = member_binary("c-node");
  ...
  for (const auto &a : {"c", "go"}) {
    for (const auto &b : {"c", "go"}) {
      for (const auto &c : {"c", "go"}) {
...
run_report(const std::string &name,
  ...
  for (size_t phase_index = 0;
    ...
    const auto &transport =
        holons::composite::TransportCoverageSequence[phase_index];
    ...
    for (int tick = 1; tick <= kRunTicks; ++tick) {
      ...
      auto result =
          run_tick(*cascade, sender, transport, members, previous, live);
      if (result.pass) {
