...
CascadeReportData run_default(const std::string &c_binary,
                              const std::string &repo_root,
                              bool emit) {
  auto run_root = make_temp_dir("observability-cascade-c-");
  emit_line(emit, "=== observability-cascade-c ===");
  emit_line(emit);
  int pass = 0, fail = 0;
  std::vector<std::string> failures;
  std::string previous;
...
CascadeReportData run_live_stream(const std::string &c_binary,
                                  const std::string &repo_root,
                                  bool emit) {
  auto run_root = make_temp_dir("observability-cascade-c-live-");
  emit_line(emit, "=== observability-cascade-c --live-stream ===");
  emit_line(emit);
...
MultiPatternReportData run_multi_pattern(const std::string &c_binary,
  ...
  emit_line(emit, "=== observability-cascade-c (multi-pattern) ===");
  emit_line(emit);
  int pass = 0, fail = 0;
...
int main(int argc, char **argv) {
  try {
    ...
    if (multi_pattern) {
      ...
      run_default(c_binary, repo_root, true);
    }
  } catch (const std::exception &error) {
    std::cerr << "\nFAIL: " << error.what() << "\n";
    return 1;
