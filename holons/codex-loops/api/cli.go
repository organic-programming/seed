...
import (
	...
	serverpkg "github.com/organic-programming/codex-loops/internal/server"
...
func newRootCommand(stdout io.Writer, stderr io.Writer) *cobra.Command {
	...
	root.AddCommand(&cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			_, err := fmt.Fprintf(stdout, "codex-loops %s\n", VersionString())
			return err
		},
	})
	if completion := findCommand(root, "completion"); completion != nil {
		completion.Short = "Generate shell completion scripts for codex-loops"
...
func newRunCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "run",
		Short: "Process the queue: feed briefs to Codex, validate with ader gates",
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := commandViper(cmd)
			req := &codexloopsv1.RunRequest{
	...
	cmd.Flags().String("root", "", "ader/codex-loops root directory (default: auto-detect)")
...
func newEnqueueCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resp, err := Enqueue(&codexloopsv1.EnqueueRequest{
	...
	cmd.Flags().String("root", "", "ader/codex-loops root directory")
...
func newListCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := commandViper(cmd)
			resp, err := List(&codexloopsv1.ListRequest{Root: cfg.GetString("list.root")})
			if err != nil {
				return err
			}
...
func newStatusCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := commandViper(cmd)
			resp, err := Status(&codexloopsv1.StatusRequest{Root: cfg.GetString("status.root")})
			if err != nil {
				return err
			}
			_, err = io.WriteString(stdout, resp.GetReportMarkdown())
...
func newDropCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := commandViper(cmd)
			resp, err := Drop(&codexloopsv1.DropRequest{
...
func newResumeCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := commandViper(cmd)
			resp, err := Resume(&codexloopsv1.ResumeRequest{Root: cfg.GetString("resume.root")})
			if err != nil {
				return err
			}
			_, err = fmt.Fprintf(stdout, "report: %s\n", resp.GetReportPath())
...
func newSkipCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := commandViper(cmd)
			resp, err := Skip(&codexloopsv1.SkipRequest{Root: cfg.GetString("skip.root")})
			if err != nil {
				return err
			}
...
func newAbortCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := commandViper(cmd)
			resp, err := Abort(&codexloopsv1.AbortRequest{Root: cfg.GetString("abort.root")})
			if err != nil {
				return err
			}
...
func newReEnqueueCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := commandViper(cmd)
			resp, err := ReEnqueue(&codexloopsv1.ReEnqueueRequest{
...
func newLogCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			cfg := commandViper(cmd)
			resp, err := Log(&codexloopsv1.LogRequest{
...
func newCompletionInstallCommand(stdout io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			status, err := ensureProfileSnippet(profile, "# codex-loops CLI autocompletion", line)
			if err != nil {
				return err
			}
...
func commandViper(cmd *cobra.Command) *viper.Viper {
	cfg := viper.New()
	cfg.SetEnvPrefix("CODEX_LOOPS")
	cfg.SetEnvKeyReplacer(strings.NewReplacer(".", "_", "-", "_"))
	cfg.AutomaticEnv()
	for _, flag := range []string{"root", "dry-run", "max-retries", "from-cookbook", "deferred"} {
...
func flattenStatus(status *codexloopsv1.StatusSnapshot) []*codexloopsv1.SlotSummary {
	if status == nil {
		return nil
	}
	var items []*codexloopsv1.SlotSummary
...
func printSlotLine(stdout io.Writer, summary *codexloopsv1.SlotSummary) error {
	if summary == nil {
		return nil
	}
...
func completionInstallTarget(args []string, shellPath string, home string, overrideProfile string) (shell string, profile string, line string, err error) {
	...
	switch shell {
	case "zsh":
		line = `eval "$(codex-loops completion zsh)"`
	...
	case "bash":
		line = `source <(codex-loops completion bash)`
		if strings.TrimSpace(overrideProfile) != "" {
			profile = overrideProfile
		} else {
