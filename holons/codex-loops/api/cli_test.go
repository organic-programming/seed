...
func TestRunCLIVersion(t *testing.T) {
	...
	if got := strings.TrimSpace(stdout.String()); !strings.HasPrefix(got, "codex-loops ") {
		t.Fatalf("version output = %q, want prefix %q", got, "codex-loops ")
	}
	if stderr.Len() != 0 {
...
func TestRunCommandShape(t *testing.T) {
	...
	if run.Flags().Lookup("root") == nil || run.Flags().Lookup("dry-run") == nil || run.Flags().Lookup("max-retries") == nil {
