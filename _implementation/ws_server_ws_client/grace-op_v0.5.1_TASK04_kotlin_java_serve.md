# TASK04 — Kotlin + Java: Wire stdio/unix/mem Dispatch in `Serve`
...
all schemes. The gap is that `Serve` never dispatches `unix`, `stdio`, or `mem`
...
is Transport.Listener.Mem   -> startMemServer(listener.runtime, builder)
...
- **`mem`** — `InProcessServerBuilder.forName(listenUri)` + matching `InProcessChannelBuilder`. The `Transport.MemListener` is already the JVM in-process socket pair; redirect to gRPC in-process channel for correct framing.
