...
func parseFlags() (options, error) {
	...
	insecureTLS := flag.Bool("insecure-tls", false, "skip TLS certificate verification for wss://")
	flag.Parse()
...
func dial(
	...
	if strings.HasPrefix(args.uri, "ws://") || strings.HasPrefix(args.uri, "wss://") {
		...
		conn, cleanup, err := dialWebSocket(ctx, wsURI, args.serverSDK, args.insecureTLS)
...
func dialWebSocketWithRetry(ctx context.Context, wsURI string, insecureTLS bool) (*grpc.ClientConn, error) {
	var lastErr error
	deadline := time.Now().Add(3 * time.Second)
	options := grpcclient.WebSocketDialOptions{}
	...
	for {
		conn, err := grpcclient.DialWebSocketWithOptions(ctx, wsURI, options)
		if err == nil {
			return conn, nil
		}
		lastErr = err
...
func dialWebSocket(ctx context.Context, wsURI, serverSDK string, insecureTLS bool) (*grpc.ClientConn, func(), error) {
	...
	if parsed.Port() == "0" {
		return dialEphemeralWebSocket(ctx, serverSDK, insecureTLS)
	}

	conn, err := dialWebSocketWithRetry(ctx, wsURI, insecureTLS)
	return conn, nil, err
}

func dialEphemeralWebSocket(ctx context.Context, serverSDK string, insecureTLS bool) (*grpc.ClientConn, func(), error) {
	...
	conn, err := dialWebSocketWithRetry(ctx, lis.Addr().String(), insecureTLS)
	if err != nil {
		server.Stop()
		_ = lis.Close()
		return nil, nil, err
