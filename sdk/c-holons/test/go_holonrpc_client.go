...
const (
	transportModeWebSocket transportMode = iota + 1
	transportModeHTTP
)
...
func main() {
	...
	startedAt := time.Now()
	mode, targetURL, err := normalizeTransportURL(args.url)
	if err != nil {
		fmt.Fprintln(os.Stderr, err)
		os.Exit(2)
	}
	...
	if mode == transportModeHTTP {
		runHTTPMode(args, startedAt, timeout)
		return
	}
...
func runHTTPMode(args options, startedAt time.Time, timeout time.Duration) {
	client := holonrpc.NewHTTPClient(args.url)

	if args.bidirectional {
		fmt.Fprintln(os.Stderr, "HTTP+SSE does not support bidirectional calls")
		os.Exit(2)
	}

	if args.connectOnly {
		ctx, cancel := context.WithTimeout(context.Background(), timeout)
		_, err := client.Invoke(ctx, "rpc.heartbeat", map[string]any{})
		cancel()
		if err != nil {
			fmt.Fprintf(os.Stderr, "dial failed: %v\n", err)
			os.Exit(1)
	...
	if args.stream {
		ctx, cancel := context.WithTimeout(context.Background(), timeout)
		events, err := client.Stream(ctx, args.method, args.params)
		cancel()
		if err != nil {
			if code, ok := rpcErrorCode(err); ok && containsInt(args.expectedErrorIDs, code) {
				mustWriteJSON(map[string]any{
		...
		if len(args.expectedErrorIDs) > 0 {
			fmt.Fprintf(os.Stderr, "expected one of error codes %v, but stream succeeded\n", args.expectedErrorIDs)
			os.Exit(1)
		}
		if len(events) == 0 || events[len(events)-1].Event != "done" {
			fmt.Fprintf(os.Stderr, "unexpected stream events: %+v\n", events)
			os.Exit(1)
		}
	...
	ctx, cancel := context.WithTimeout(context.Background(), timeout)
	out, err := client.Invoke(ctx, args.method, args.params)
	cancel()
	if err != nil {
		if code, ok := rpcErrorCode(err); ok && containsInt(args.expectedErrorIDs, code) {
			mustWriteJSON(map[string]any{
...
func parseFlags() (options, error) {
	...
	if out.url == "" {
		return options{}, fmt.Errorf("usage: go_holonrpc_client.go <ws://host:port/rpc|wss://host:port/rpc|http://host:port/api/v1/rpc|rest+sse://host:port/api/v1/rpc> [flags]")
	}
...
func normalizeTransportURL(raw string) (transportMode, string, error) {
	trimmed := strings.TrimSpace(raw)
	switch {
	case strings.HasPrefix(trimmed, "ws://"), strings.HasPrefix(trimmed, "wss://"):
		return transportModeWebSocket, trimmed, nil
	case strings.HasPrefix(trimmed, "http://"), strings.HasPrefix(trimmed, "https://"):
		return transportModeHTTP, strings.TrimRight(trimmed, "/"), nil
	case strings.HasPrefix(trimmed, "rest+sse://"):
		return transportModeHTTP, strings.TrimRight("http://"+strings.TrimPrefix(trimmed, "rest+sse://"), "/"), nil
	default:
		return 0, "", fmt.Errorf("unsupported Holon-RPC transport: %s", raw)
	}
}
