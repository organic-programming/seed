...
- **`Discover`** ⚠️: Scans layers, returns all matches — `(scope, expression?, root, specifiers) → DiscoverResult` — see [DISCOVERY.md](../DISCOVERY.md). NOT IMPLEMENTED.
- **`resolve`** ⚠️: First match — `(scope, expression, root, specifiers) → HolonRef` — thin helper over `Discover`. NOT IMPLEMENTED.
