...
export class HolonError extends Error {
     ...
     * @param {unknown} [data] - optional structured error data
     */
...
export class HolonClient {
    ...
    /**
     * @param {string} url - WebSocket URL, e.g. "ws://localhost:8080/api/v1/rpc"
    ...
    /**
     * Register a browser-side handler for server-initiated calls.
     *
    ...
    /**
     * Invoke a method on the remote server.
     *
    ...
    async #handleRequest(msg) {
        ...
        try {
            ...
            const data = err instanceof HolonError ? err.data : undefined;
            ws.send(JSON.stringify({
                jsonrpc: "2.0",
                id: msg.id,
    ...
    #handleResponse(msg) {
        ...
        if (msg.error) {
            pending.reject(new HolonError(msg.error.code, msg.error.message, msg.error.data));
            return;
        }
